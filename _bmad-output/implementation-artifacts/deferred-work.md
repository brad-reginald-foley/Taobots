...
<!-- Below: surfaced by the Story 1.3 code review (2026-08-17). -->

- source_spec: `_bmad-output/implementation-artifacts/spec-1-3-earth-consuming-structural-repair-legs-first.md`
  summary: The workshop CSV's repair columns and the inspector's repair labels can both go dead with the suite green.
  evidence: Hard-coding `repair_earth_spent`, `repair_integrity_gained`, per-leg `mass`, `repair_earth` and `repair_gain` to `0.0` in `WorkshopLogger.log_tick` leaves 564 tests passing — and those columns exist precisely so the crossing can be audited off the CSV (`earth == gain × mass × rate`). Likewise forcing both display branches in `renderer.py`'s `_earth_label` and `_integrity_label` to the not-repairing case passes: the `FLOOR` label, whose whole job is to distinguish "correctly refusing to heal" from "repair is broken", is entirely unpinned. The chi equivalents (`chi_*` columns, `_deficit_label`) each got parsed-row and reference-rendered tests in Story 1.2; repair should get the same treatment.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-3-earth-consuming-structural-repair-legs-first.md`
  summary: The invariant harness gained a `repairing legs` scenario but no seventh invariant, so the chi→part crossing is never asserted over a long run.
  evidence: The crossing is asserted only in `tests/test_repair.py`, on single calls to `_repair_parts()`. The harness's six invariants are bounds checks plus an essence check armed only inside `chi.convert`, so across 3000 ticks of a bot cycling between damage and repair nothing checks that Earth debited equals integrity gained. Measured: the scenario does reach the regime (656 of 1917 ticks with a part strictly between 0 and 1), so the run exists — it is only unobserved. Adding the crossing as a per-tick invariant is the natural home, and would also make the scenario's own comment true.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-4-verify-the-leg-integrity-round-trip-in-workshop.md`
  summary: A workshop run spanning a death splices several bots into one apparently continuous series; **decided not to mark it** (Brad, 2026-08-17) — the `entity_id` column is sufficient.
  evidence: Observed in `logs/workshop_workshop_20260817T130106_672.csv`: 2342 rows, entity ids 27/28/29, with the bot replaced at rows 216 and 2095. Leg integrity appears to dip and fully recover across a splice while the repair columns record zero, because the recovery is a fresh bot's legs. Recorded so the reading is not rediscovered as a bug: group by `entity_id` before treating a workshop CSV as one life. Both derivation sweeps already do — `derive_chi_laws.py` keys every measurement by entity id and counts deaths, `derive_repair_laws.py` breaks on death — and Story 1.4's exit-criterion evidence was verified single-bot. The panel gives no equivalent signal beyond `Bot #N` and a reset `Age`, which is the residual risk accepted here.
